...
def getGAEZ_drainage_req(CROP_ID, Input_Level_List):
    try:
        conn = getDataStore_Connection()
        cur = conn.cursor()
        sql = 'SELECT CROP_ID, CROP, input_level, SQI_code, PSCL_HWSD, DrainNum, Drain, score FROM  GAEZ_drainage_req_rf WHERE CROP_ID=\'' + CROP_ID + '\' AND input_level IN ('  + ','.join(map(str, Input_Level_List)) + ')'
        cur.execute(sql)
        results = cur.fetchall()
        data = pd.DataFrame(list(results))
        data.columns = ['CROP_ID', 'CROP', 'input_level', 'SQI_code', 'PSCL_HWSD', 'DrainNum', 'Drain', 'score']
        return data
        conn.close()
    except Exception as err:
        print(err)
...
## ---------------------------------------------------------------------------------------------------------------------
    ...
    if plot_data is not None:
        ...
        if soil_df_slice is not None:
            ...
            p_hz_data = pd.concat([hz_depb, txt_d, txt_id, txt_grp, rf_d], axis=1)
